...
Основной двигатель: бесколлекторный, размера 2205, 2200 оборотов. Управляется через регулятор двигателя ШИМ-сигналом с ESP32. Возможен передний и задний ход.
Сервопривод поворота колес: MG9, 9 грамм
...
Система питания:
В роботе используется одна батарея LiPo 2s (7.4V) 1200mAh. Подключена через выключатель. От нее через регулятор двигателя питается собственно двигатель. А так же через DC-DC 
...
Схема подключения системы питания к контроллерам, двигателям, а так же сенсоров к контроллеру находится в папке schemes.
Для уменьшения количества проводов между контроллерами и датчиками была вытравлена на текстолите плата, на которую уже паялись разъемы для контроллеров и подключались датчики.
Ее схема так же находится в папке schemes.
